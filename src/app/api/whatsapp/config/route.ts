      ...
      return NextResponse.json(
        {
          ...
          needs_reset: true,
          message:
            'The stored access token cannot be decrypted with the current ENCRYPTION_KEY. This usually means the key changed, or it differs between environments (local vs your host vs Vercel). Click "Reset Configuration" below, then re-save.',
        },
        { status: 200 }
      )
    }
...
export async function POST(request: Request) {
  try {
    ...
    // This CRM is single-tenant-per-WhatsApp-number — letting two accounts
