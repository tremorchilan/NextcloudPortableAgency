...
//      reverse proxy in front of the app: a managed Node host,
//      Vercel, Cloudflare, nginx. This is what makes
...
//   typical proxied deploy (Vercel / a managed host / Cloudflare) the
...
//   on the list falls through to the brand fallback URL with a
...
//
// Previous implementation hard-defaulted to the upstream marketing
// domain. Forks that didn't set `NEXT_PUBLIC_SITE_URL` got invite
// links pointing at a site that 404s on `/join/<token>`. This
// resolution chain removes the foot-gun by falling back to the
// deploy's own `NEXT_PUBLIC_BRAND_SITE_URL`.
...
function getBaseUrl(request: Request): string {
  ...
  // Last-resort fallback: the deploy's own brand URL (e.g. the
  // agency's site). Defaults to a placeholder so a misconfigured
  // deploy never points invite links at an unrelated domain.
  return process.env.NEXT_PUBLIC_BRAND_SITE_URL?.trim() || "https://crm.example.com";
}
